...
class PoeNinjaUnitTest
{
    ...
    {
        val currencyCall = testPoeNinjaService?.getCurrencyOverview("Delirium", overviewNamesByType?.get(Overview.Type.CURRENCY) ?: "")
        val currencyOverview = currencyCall?.execute()?.body()

        assertNotNull(currencyOverview)

        val fragmentCall = testPoeNinjaService?.getCurrencyOverview("Delirium", overviewNamesByType?.get(Overview.Type.FRAGMENT) ?: "")

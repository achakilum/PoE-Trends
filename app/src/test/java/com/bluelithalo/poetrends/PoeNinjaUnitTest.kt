...
class PoeNinjaUnitTest
{
    ...
    {
        val currencyCall = testPoeNinjaService?.getCurrencyOverview("Harvest", overviewNamesByType?.get(Overview.Type.CURRENCY) ?: "")
        val currencyOverview = currencyCall?.execute()?.body()

        assertNotNull(currencyOverview)

        val fragmentCall = testPoeNinjaService?.getCurrencyOverview("Harvest", overviewNamesByType?.get(Overview.Type.FRAGMENT) ?: "")
